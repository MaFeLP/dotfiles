...
function init() {
  ...
  # Select the language
  local language=$(whiptail --title 'Language' --backtitle 'latex.sh' --notags --menu 'Select a language' 0 0 0 german Deutsch english English spanish "Español" 3>&1 1>&2 2>&3 | sed 's/"//g')
  ...
  packages+=("\usepackage{babel}" "babel - langage optimizations" "on")
...
EOF
  ...
  cat > "$filename" <<EOF
\\documentclass[a4paper,$language]{article}
